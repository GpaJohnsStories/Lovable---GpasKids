-- STAGE 1: Secure Admin Migration with Safety Nets (Fixed)
-- This creates the new secure admin system while keeping the old one working

-- 1. Create emergency bypass function (temporary safety net)
...
-- 2. Create enhanced admin check function with fallback
...
-- 3. Enable RLS on comments table with dual admin system
...
-- 4. Secure stories table - restrict write access to admins only
...
-- 5. Create audit table for tracking admin actions
...
-- 6. Create emergency reset function (use only if locked out)
...
-- 7. Create function to promote user to admin (for emergency use)
CREATE OR REPLACE FUNCTION public.emergency_promote_admin(user_email text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
