...
interface StoryVotingProps {
  ...
  onVoteUpdate: (newCounts: {
    ...
    ok_count: number;
  }, newVote: 'thumbs_up' | 'thumbs_down' | 'ok' | null) => void;
}
const StoryVoting = ({
  storyId,
  storyCode,
  ...
  const handleVote = async (voteType: 'thumbs_up' | 'thumbs_down' | 'ok') => {
    ...
    try {
      ...
      const {
        error: updateError
      } = await supabase.from('stories').update(newCounts).eq('id', storyId);
      if (updateError) {
  ...
  return <div className="flex flex-col items-center space-y-4 p-6 bg-amber-50 rounded-lg border border-amber-200">
      <h3 className="text-2xl font-bold text-orange-800 mb-2 font-serif italic">How did you like this story?</h3>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4">
        ...
        <div className="flex space-x-4">
          ...
          <Button variant="ghost" onClick={() => handleVote('ok')} disabled={isVoting} className={getButtonClass('ok')}>
          ...
          <Button variant="ghost" onClick={() => handleVote('thumbs_down')} disabled={isVoting} className={getButtonClass('thumbs_down')}>
        ...
        <Link to={`/make-comment?storyCode=${encodedStoryCode}`}>
          <Button className="bg-blue-500 hover:bg-blue-600 text-white h-12 w-80 rounded-full flex items-center justify-center transition-all duration-300 text-lg font-medium">
            <span className="text-base leading-tight text-center font-bold">Please Tell Us About Your Vote</span>
          </Button>
        </Link>
      </div>
    </div>;
