...
const HeaderContent = ({ isHomePage, isAdminPage = false }: HeaderContentProps) => {
  ...
  const { iconUrl: buddyIconUrl, isLoading: buddyLoading, error: buddyError } = useCachedIcon('ICO-HL2.png');
  const { iconUrl: menuIconUrl, isLoading: menuLoading, error: menuError } = useCachedIcon('ICO-MU2.png');
  const { iconUrl: safeForKidsIconUrl, isLoading: sfkLoading, error: sfkError } = useCachedIcon('ICO-SFK.png');
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button 
                  ...
                  className="group relative z-10 bg-transparent hover:bg-transparent rounded-full p-1 flex flex-col items-center justify-center text-center w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 min-w-14 sm:min-w-16 md:min-w-20 flex-shrink-0 transform hover:scale-105 hover:-translate-y-1 transition-all duration-200 cursor-pointer active:scale-95 active:translate-y-1"
                >
                  {hgjLoading && (
