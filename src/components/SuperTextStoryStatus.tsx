...
const SuperTextStoryStatus: React.FC<SuperTextStoryStatusProps> = ({
  story,
  publicationStatusCode = 5,
  onStatusChange
}) => {
  ...
  const statusLabels = ["0 - Save & Pub, App & Rev", "1 - Save & Pub, App Only", "2 - Save, NO PUB, No CoP", "3 - Save, NO Gpa APR", "4 - Save, Formatting", "5 - NOT SAVED"];
  return <Card className="h-fit relative w-fit max-w-xs" style={{
    borderColor: '#3b82f6',
    borderWidth: '2px'
  }}>
      <CardHeader className="p-0">
        ...
        textShadow: '2px 2px 0px #B8860B, 4px 4px 0px #8B6914, 6px 6px 0px #654321, 8px 8px 15px rgba(0,0,0,0.4)'
          ...
          <span>Set Publication Status</span>
          <span>BEFORE Saving</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="p-0">
        <div className="space-y-0">
          {statusLabels.map((label, index) => <button key={index} onClick={() => onStatusChange?.(index)} style={getStatusButtonStyle(index)}>
              {label}
            </button>)}
        </div>
      </CardContent>
